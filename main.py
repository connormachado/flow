...
def findPathsReturn(board, path, curX, curY, endX, endY, lastMove):
    paths = [] #all paths associated with this specific line
    path = [] #The current path that the line is following
    lastMove = "" #Last move the line took
    lastThreeMoves = "" #Last three moves the line took
    notAllowed = ["urd", "uld", "rdl", "rul", "lur", "ldr", "dru", "dlu"] #Moves not allowed (gets rid of redundant moves)
...
def solve():
    ...
    #Actual Solving paths algorithm
    #Does not finish until it is true
    while True:
...
def drawCircles(win, posList, ss):
    colors = [c.ORANGE, c.RED, c.YELLOW, c.GREEN, c.BLUE, c.TEAL, c.PURPLE, c.PEACH, c.PINK, c.PINKER]
    p = 0
